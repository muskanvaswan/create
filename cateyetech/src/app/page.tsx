...
function Hero() {
  return (
    <section className="relative overflow-hidden bg-ink-900">
      ...
      <div
        className="absolute -left-32 top-10 h-[26rem] w-[26rem] rounded-full bg-brand-500/20 blur-3xl"
        aria-hidden="true"
      />
      <div
        className="absolute -right-24 bottom-0 h-[22rem] w-[22rem] rounded-full bg-brand-700/25 blur-3xl"
        aria-hidden="true"
      />
      <div className="container-page relative py-20 sm:py-28 lg:py-32">
        <div className="grid items-center gap-14 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="animate-fade-up">
            ...
            <h1 className="heading-xl mt-5 max-w-[16ch] text-white">
              Technology leadership and security your business can{" "}
              <span className="text-brand-300">stand behind</span>.
            </h1>
            ...
            <dl className="mt-14 grid max-w-lg grid-cols-2 gap-x-8 gap-y-6 border-t border-white/10 pt-8 sm:grid-cols-4 sm:gap-x-4">
              {stats.map((stat) => (
                <div key={stat.label}>
                  ...
                  <dd className="mt-1 text-[0.78rem] leading-snug text-ink-300">
                    {stat.label}
                  </dd>
                </div>
              ))}
          ...
          <div className="relative hidden lg:block">
            <div className="relative rounded-3xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur-sm">
              <div className="flex items-center justify-between">
                <Logo className="h-10 w-10 text-brand-300" />
                <span className="rounded-full bg-brand-400/15 px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.14em] text-brand-200">
                  Six spectrums
                </span>
              </div>
        ...
        {capabilities.map((capability) => {
          ...
          return (
            <Card key={capability.title}>
              ...
              <h3 className="mt-5 text-[1.05rem] font-semibold text-ink-900">
                {capability.title}
              </h3>
...
function Spectrums() {
  return (
    <Section className="border-y border-ink-100 bg-ink-50/50">
      ...
      />
      <div className="mt-14 grid gap-px overflow-hidden rounded-2xl border border-ink-200/70 bg-ink-200/70 sm:grid-cols-2 lg:grid-cols-3">
        {spectrums.map((spectrum, index) => (
          ...
          >
            ...
            <h3 className="mt-3 text-[1.05rem] font-semibold text-ink-900">
              {spectrum.title}
            </h3>
            <p className="mt-2.5 text-[0.93rem] leading-relaxed text-ink-500">
    ...
    <section className="relative overflow-hidden bg-ink-900">
      ...
      <div className="container-page relative py-20 sm:py-24">
        <figure className="mx-auto max-w-3xl text-center">
          <Logo className="mx-auto h-10 w-10 text-brand-400" />
          <blockquote className="mt-7 text-2xl font-medium leading-snug tracking-[-0.01em] text-white sm:text-[1.85rem]">
