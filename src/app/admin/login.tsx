...
export function AdminLogin({ hasPasskey }: Props) {
    ...
    <div className="flex h-full flex-col items-center justify-center gap-4 px-6 text-center">
      ...
      <p className="max-w-sm text-sm text-neutral-500 dark:text-neutral-400">
        {hasPasskey
          ...
          : "Enter your password to unlock the editor. You can also create a passkey for faster sign-in on this device."}
      </p>

      <form
        onSubmit={signInWithPassword}
        className="flex w-full max-w-[16rem] flex-col items-center gap-4"
      >
        <input
          type="password"
          value={password}
          ...
          className="w-full rounded-full border border-neutral-300 bg-white/60 px-4 py-2 text-center text-[15px] outline-none focus:border-[#e0a30c] dark:border-neutral-600 dark:bg-black/30 dark:focus:border-[#a17321]"
        />
        <button
          type="submit"
          disabled={busy !== null}
          className="w-full rounded-full bg-[#e0a30c] px-6 py-2.5 text-[15px] font-semibold text-white shadow-md hover:bg-[#c89209] disabled:opacity-50 dark:bg-[#a17321] dark:hover:bg-[#b5832a]"
        >
          {busy === "password" ? "Signing in…" : "Sign in"}
        </button>
      </form>

      <div className="flex w-full max-w-[16rem] items-center gap-3 text-xs text-neutral-400 dark:text-neutral-500">
      ...
      <button
        ...
        disabled={busy !== null}
        className="w-full max-w-[16rem] rounded-full border border-neutral-300 bg-white/40 px-6 py-2.5 text-[15px] font-semibold text-neutral-700 shadow-sm hover:bg-white/70 disabled:opacity-50 dark:border-neutral-600 dark:bg-black/20 dark:text-neutral-200 dark:hover:bg-black/40"
      >
        {busy === "passkey"
          ? "Waiting for passkey…"
          : hasPasskey
